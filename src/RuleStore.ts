...
export class RuleStore {
  ...
  private addGenericRule(map: Map<string, StoredRule>, type: GenericRuleType, metadata: RuleMetadata, originalRule: string): void {
    // Map 'parameter' type to 'removeparam' stat key if necessary
    const statType = (type === 'parameter' ? 'removeparam' : type) as keyof RuleStats;

    // Check if the mapped statType is valid before proceeding
    if (!(statType in this.stats)) {
        console.error(`[addGenericRule] Invalid or unhandled stat type: '${statType}' (mapped from '${type}') for rule: ${originalRule}`);
        this.stats.invalid++;
        return;
    }
    ...
    if (!existingRule) {
      ...
      if (this.stats[statType] !== undefined) {
          ...
          console.warn(`[addGenericRule] Stat key '${statType}' not found for incrementing.`);
      }
    } else {
  ...
  addRule(originalRule: string, sourceName: string = 'unknown'): void {
    ...
    const type: RuleClassificationType = this.ruleProcessor.classifyRule(trimmedRule);
    ...
    try {
      ...
      switch (type) {
        ...
        case 'hint':
          ...
          if (statKey in this.stats && this.stats[statKey] !== undefined) {
            this.stats[statKey]++;
             ...
             console.warn(`[addRule] Stat key '${type}' not found or undefined for incrementing.`);
          }
        ...
        default:
          ...
          console.warn(`[RuleStore.addRule] Unhandled or unknown type: ${type} for rule: ${trimmedRule}`);
          this.stats.invalid++;
          // No targetMap assigned, will skip addGenericRule call below
          break; // Exit switch
      }
      ...
      if (targetMap && isGenericHandledType) {
          ...
          this.addGenericRule(targetMap, type as GenericRuleType, metadata, trimmedRule);
      }

    } catch (error) {
      console.error(`[RuleStore.addRule] Error processing rule: ${trimmedRule}`, error);
      this.stats.invalid++;
    }
  }

  // Handles blocking and unblocking rules (domain/pattern or hash keyed)
  private handleStandardRule(originalRule: string, type: 'blocking' | 'unblocking', metadata: RuleMetadata): void {
      let key: string | null; // <<< Allow null
      let isHashKey = false;

      // Determine if it should use a hash key (global modifiers, etc.)
      // Refine this logic based on RuleProcessor's capabilities
      if (originalRule.includes('$') && !originalRule.includes('#') && !originalRule.includes('##')) {
          // Simple check: if '$' is present but not cosmetic/scriptlet markers, assume hash key needed
          key = this.generateHash(originalRule);
          isHashKey = true;
      } else {
          key = this.ruleProcessor.cleanDomainPattern(originalRule); // <<< This can return null
      }

      if (!key) { // <<< This check correctly handles the null case
          console.warn(`[handleStandardRule] Could not determine key for ${type} rule: ${originalRule}`);
          this.stats.invalid++;
          return;
      }

      const ruleMap = type === 'unblocking' ? this.unblockingRules : this.blockingRules;
      ...
      if (existingRule && !isHashKey) {
          console.warn(`[handleStandardRule] Overwriting rule with same key '${key}' but different content. Old: ${existingRule.originalRule}, New: ${originalRule}`);
  ...
  private handleCosmeticRule(originalRule: string, metadata: RuleMetadata): void {
      const selector = this.ruleProcessor.extractSelector(originalRule);

      if (!selector) {
          // Check if it might be a misclassified scriptlet
          if (!originalRule.includes('#$#') && !originalRule.includes('#%#')) {
              console.warn(`[handleCosmeticRule] Could not extract selector for rule: ${originalRule}`);
              this.stats.invalid++;
          } else {
              console.warn(`[handleCosmeticRule] Rule with #$#/#%# classified as cosmetic, expected scriptlet: ${originalRule}`);
              this.stats.invalid++; // Count as invalid classification
          }
          return;
      }
      ...
      if (existingRule) {
           console.warn(`[handleCosmeticRule] Overwriting rule with same selector '${key}' but different content. Old: ${existingRule.originalRule}, New: ${originalRule}`);
      }
